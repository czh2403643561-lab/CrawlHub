(() => {
  ...
  function isSourceImage(url) {
    try {
      ...
      return /(?:tiktokcdn|ibytedtos|byteimg)/.test(parsed.hostname) && !/(?:logo|icon|avatar|static)/.test(target);
    } catch { return false; }
  }
  function isVisible(image) {
    const rect = image.getBoundingClientRect(), style = getComputedStyle(image);
    return style.display !== "none" && style.visibility !== "hidden" && Number(style.opacity || 1) > 0 && rect.width > 80 && rect.height > 80 && rect.bottom > 0 && rect.right > 0 && rect.top < innerHeight && rect.left < innerWidth;
  }
  function isTopProductMedia(image) {
    if (!isProductPage() || !(image instanceof HTMLImageElement) || !isVisible(image) || !isSourceImage(imageUrl(image))) return false;
  ...
  function ensureHoverHost() {
    ...
    style.textContent = "button{display:grid;place-items:center;width:42px;height:42px;border:2px solid #ff0050;border-radius:50%;background:#fff;color:#e60046;box-shadow:0 3px 12px rgba(0,0,0,.28);cursor:pointer;padding:0}button:hover{background:#fff0f4}svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:2.2}";
  ...
  function positionHoverButton() {
    if (!activeImage || !isTopProductMedia(activeImage)) { hideHoverButton(); return; }
    const rect = activeImage.getBoundingClientRect();
    hoverHost.style.left = `${Math.max(8, rect.right - 54)}px`;
    hoverHost.style.top = `${Math.max(8, rect.bottom - 54)}px`;
  ...
  function handleProductImagePointerOver(event) {
    ...
    if (!isTopProductMedia(image)) return;
  ...
  function activateResultImage(image) {
    ...
    const loaded = () => { clearTimeout(timer); imageDiagnostic("result_image_direct_loaded", image, startedAt); };
    const failed = () => { clearTimeout(timer); imageDiagnostic("result_image_direct_failed", image, startedAt); proxyResultImage(image, startedAt); };
